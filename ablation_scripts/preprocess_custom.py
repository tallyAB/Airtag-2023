...
def is_matched(event_id):
    global gt_event_ids
    
    if event_id in gt_event_ids:
        return True
    return False
...
def order_events():
    ...
    combined_list = list(zip(preprocessing_lines, event_ids))
    # Sort based on the first list
    combined_list.sort(key=lambda x: x[0])
    preprocessing_lines, event_ids = zip(*combined_list)
    
    new_preprocessing_lines = []
    pre_out_line = ',' * 19
    for i,out_line in enumerate(preprocessing_lines):
        if ',,,,,,,,,,,,,,,,,,,' not in out_line:
            # write lines out, remove adjacent duplicate entries
            if len([(i, j) for i, j in zip(out_line.split(','), pre_out_line.split(',')) if i != j]) > 1:
                if is_matched(event_ids[i]):
                    new_preprocessing_lines.append('\n' + out_line.lower().replace("\\", "/") + '-LA+')
                else:
                    new_preprocessing_lines.append('\n' + out_line.lower().replace("\\", "/") + '-LA-')
                pre_out_line = out_line
    preprocessing_lines = new_preprocessing_lines
...
def pp_audit(wf, path):
    ...
    for entry in single_lines:
        ...
        # event_id
        event_id = re.findall(r'msg=audit\([^:]*:[^:]*:[^:]*:(\d+)', entry)[0]
        ...
        # src ip&port **
        s_ip = re.findall(r' nf_src_ip=(\S+) ', entry)
        s_port = re.findall(r' nf_src_port=(\S+) ', entry)
        if s_ip:
            out_line += ',' + s_ip[0]
        else:
            out_line += ','
        ...
        d_ip = re.findall(r' nf_dst_ip=(\S+) ', entry)
        d_port = re.findall(r' nf_dst_port=(\S+) ', entry)
        if d_ip:
            out_line += ',' + d_ip[0]
        else:
            out_line += ','
        ...
        preprocessing_lines.append(out_line)
...
if __name__ == '__main__':    
    # Get file paths from args and fill list, output dir also from args, gt as well
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('-files', action='store', nargs='+')
    parser.add_argument('-gt', nargs='+', default=None)
    parser.add_argument('-o_dir', action='store')
    args = parser.parse_args()

    files = args.files
    output_dir = args.o_dir
    # Assert gt files equal to files_to_be_processed if gt specified
    if args.gt:
        if len(files) != len(args.gt):
            print("There must be an equal number of gt and input files passed to args")
            exit()
    for i,file in enumerate(files): 
        ...
        gt_lines = []
        if args.gt:
            # Open gt file and write lines to gt_event_ids_list
            with open(args.gt[i], 'r') as gt_file:
                gt_lines = gt_file.readlines()
                gt_event_ids = [int(x.strip()) for x in gt_lines]
        preprocessing_lines = []
        malicious_labels = []
        output_file = output_dir + "/" + os.path.basename(file)
        training_wf = open(output_file, 'w')
        pp_audit(training_wf, file)
        # if "linux" in file:
        #     pp_audit(training_wf, file)
        # if "windows" in file:
        #     pp_dns(training_wf, file)
        #     pp_http(training_wf, file)
        #     pp_audit_w(training_wf, file)
        order_events()
